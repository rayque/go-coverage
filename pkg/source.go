...
func GetFileWithSource(filePath string, coverage *FileCoverage) (*FileWithSource, error) {
	...
	if err != nil {
		...
		alternativePaths := []string{
			...
			filepath.Base(filePath), // Just the filename (e.g., calculator.go from example.com/pkg/calculator.go)
		}

		// If the path contains slashes, try parts after the module name
		// e.g., example.com/coveragetest/calculator.go -> calculator.go
		parts := strings.Split(filePath, "/")
		if len(parts) > 1 {
			// Try progressively shorter paths
			for i := 1; i < len(parts); i++ {
				partialPath := filepath.Join(parts[i:]...)
				alternativePaths = append(alternativePaths, partialPath)
			}
		}
